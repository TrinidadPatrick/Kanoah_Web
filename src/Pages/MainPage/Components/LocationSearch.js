...
const LocationSearch = () => {
  ...
  <input onChange={(e)=>{setSearchInput(e.target.value)}} className='text-white font-light w-[65%] text-sm md:text-lg py-5 ps-[3rem] px-6 bg-themeBlue rounded-s-4xl outline-none ' onKeyDown={(e)=>{if(e.key === "Enter"){navigate(`explore?search=${e.target.value}&longitude=${locationCoordinates.longitude}&latitude=${locationCoordinates.latitude}&rd=5`)}}} type="text" placeholder='Search for service' />
  <div className='w-1/2 relative'>
  <NearMeRoundedIcon fontSize='small' className='text-white absolute top-5 md:top-6 left-5' />
  <input onKeyDown={(e)=>{if(e.key === "Enter"){navigate(`explore?search=${searchInput}&longitude=${locationCoordinates.longitude}&latitude=${locationCoordinates.latitude}&rd=5`)}}} value={locationFilterValue} onChange={(e)=>{setLocationFilterValue(e.target.value)}} type='text' className='text-white w-full font-light text-sm md:text-lg ps-[3rem] py-5 px-6 bg-themeBlue rounded-s-4xl sm:border-s-2 border-slate-800 rounded-e-4xl outline-none' placeholder='Your Location' />
  {/* Places dropdown */}
  ...
  <button onClick={()=>{navigate(`explore?search=${searchInput}&longitude=${locationCoordinates.longitude}&latitude=${locationCoordinates.latitude}&rd=5`)}} className='absolute bg-white hidden sm:flex text-themeBlue px-2.5 lg:px-6 py-2.5 rounded-3xl top-[8px] lg:top-[11.6px] space-x-2 right-3 md:right-3'><SearchIcon /> <span className='hidden lg:block'>Search</span></button>
