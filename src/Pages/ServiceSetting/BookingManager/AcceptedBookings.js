...
const AcceptedBookings = ({ acceptedBookings, lazyLoad }) => {
    ...
    useEffect(()=>{
        set_Accepted_Bookings_Orig(acceptedBookings)
    },[acceptedBookings])

    const OpenClientInformation = (id) => {
        setIsOpen(true)
        const booking = acceptedBookings.find((booking) => booking._id === id)
    ...
    const updateStatus = async (id, status) => {
        const index = accepted_Bookings_Orig.findIndex(booking => booking._id === id)
        if(index !== -1)
        {
            const newBooking = [...accepted_Bookings_Orig]
            newBooking[index] = {...newBooking[index], ["status"] : status}
            const filtered = newBooking.filter((booking) => booking.status === "ACCEPTED")
            set_Accepted_Bookings_Orig(filtered)
            try {
  ...
  return (
    ...
    {
        accepted_Bookings_Orig?.map((booking)=>{
            const dateObject = new Date(booking.createdAt)
            const formattedDate = dateObject.toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
