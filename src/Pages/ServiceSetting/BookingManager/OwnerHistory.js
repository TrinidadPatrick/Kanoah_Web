...
const OwnerBookingHistory = ({ history, lazyLoad }) => {
  ...
  return (
    ...
    <div className='w-full h-full max-h-full flex flex-col gap-4 py-5 mt-5'>
    {
        history.map((booking)=>{
            ...
            return (
            <div key={booking._id} className={`w-full ${booking.status === "COMPLETED" ? "bg-green-50 border border-green-500" : "bg-red-50 border border-red-500"} border px-2 shadow-sm rounded-md flex justify-evenly h-fit p-1`}>
            {/* Left side */}
