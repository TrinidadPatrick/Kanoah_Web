...
const Explore = () => {
  ...
  const servicesPerPage = 2; // Number of services to display per page
  ...
  const ratingAverage = (services) => {
    return services.map((service, index) => {
      ...
      const createdAgo = years > 0 ? years + " years ago" : months > 0 ? months + " months ago" : days > 0  ? days + " days ago" : "Less than a day ago"
      return ({
        key : index,
            ...
            currentServices.map((service, index)=>(
              ...
              <div className=' px-1 py-3 w-full overflow-hidden flex flex-col justify-between space-y-5'>
                ...
                <div className='w-full overflow-hidden'>
                  ...
                  <p className='text-xs text-gray-400 ml-1 mt-1'>{service.createdAgo}</p>
                </div>
                {/* Reviews */}
